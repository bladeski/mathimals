...
export class LevelSelectComponent extends BaseComponent {
  ...
  constructor() {
    ...
    DomHelper.addEventListener('button', 'click', this.onClickPlay.bind(this), this.component);
    ...
    this.subs.push(
      combineLatest([
        ...
        const difficultyEl = this.component.querySelector('#difficulty') as HTMLSelectElement;
        const operatorEl = this.component.querySelector('#operator') as HTMLSelectElement;
  ...
  private onClickComponent(event: MouseEvent) {
    if ((event.target as HTMLElement).nodeName === 'FORM' || (event.target as HTMLElement).closest('form')) {
      return;
    }
  ...
  private onClickPlay() {
    ...
    StateService.updateCurrentDifficulty(parseInt(formData.get('difficulty') as string) as Difficulty);
