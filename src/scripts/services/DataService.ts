...
import { Difficulty, EndangeredStatus, Operator, StateProperty } from '../enums';
...
class DataService {
  ...
  public getAnimals() {
    return new Promise<Animal[]>(res => res(Animals.map(animal => ({
      ...animal,
      cost: DataService.getAnimalCost(animal)
    }))));
  }

  public getCurrentDifficulty() {
    return new Promise<Difficulty>(res => 
      res(UserStorageService.getSetting(StateProperty.CURRENT_DIFFICULTY) as Difficulty || Difficulty.EASY)
    );
  }

  public getCurrentOperator() {
    return new Promise<Operator>(res => 
      res(UserStorageService.getSetting(StateProperty.CURRENT_OPERATOR) as Operator || Operator.ADDITION)
    );
  }
  ...
  public getLevels() {
    return new Promise<Levels>(res => res(UserStorageService.getSetting(StateProperty.LEVELS) as Levels || new Levels())
  );
  }

  public getZooAreas() {
    return new Promise<ZooArea[]>(res => res(ZooAreas.map(zooArea => ({
      ...zooArea,
      ...(UserStorageService.getSetting(StateProperty.ZOO_AREAS) as ZooArea[]).find(area => area.id === zooArea.id)
    }))));
  }

  public getZooCoins() {
    return new Promise<number>(res => res(UserStorageService.getSetting(StateProperty.ZOO_COINS) as number));
  }

  public updateCurrentDifficulty(difficulty: Difficulty) {
    UserStorageService.updateSetting(StateProperty.CURRENT_DIFFICULTY, difficulty);
  }
